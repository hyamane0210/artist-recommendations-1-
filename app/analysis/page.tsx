...
// Tabsコンポーネントは不要になったため削除
import { Loader2, Heart, Share2, UserCircle2, Users, BarChart2, Sparkles } from "lucide-react"
            ...
            <CardContent>
              <div className="flex flex-wrap gap-4 justify-center">
                <Button
                  ...
                  onClick={async () => {
                    try {
                      // Web Share APIを使用せず、直接クリップボードにコピー
                      const shareText = `${user?.name || "私"}の性格タイプは「${personalityType?.emoji} ${personalityType?.title}」です！\n${personalityType?.description}\n#MyProject #性格診断 #好み分析\n${window.location.href}`
                      await navigator.clipboard.writeText(shareText)
                      alert("診断結果をクリップボードにコピーしました！\nSNSなどに貼り付けて共有できます。")
                    } catch (error) {
                      console.error("クリップボードへのコピーに失敗しました:", error)
                      alert("共有できませんでした。テキストを手動でコピーしてください。")
                    }
                  }}
                >
