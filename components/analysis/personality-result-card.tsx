...
export function PersonalityResultCard({
  ...
  const getSafeColor = (color: string): string => {
    // 色が有効なHEXカラーコードかどうかをチェック
    const isValidHex = /^#([0-9A-F]{3}){1,2}$/i.test(color)

    // 有効なHEXカラーコードの場合はそのまま返す、そうでなければデフォルト色を返す
    return isValidHex ? color : "#454545"
  }
  ...
  const handleShare = async (platform: string) => {
    ...
    try {
      // グラデーションの問題を回避するために、一時的に背景色を単色に変更
      const headerElement = element.querySelector('[style*="backgroundColor"]') as HTMLElement
      let originalStyle = ""

      if (headerElement) {
        originalStyle = headerElement.getAttribute("style") || ""
        // 安全な色を使用
        const safeColor = getSafeColor(personalityType.color)
        headerElement.style.backgroundColor = safeColor
      }
      ...
      // 元のスタイルに戻す
      if (headerElement && originalStyle) {
    ...
    <div className="space-y-4">
      ...
      >
        <div className="relative">
          <div className="h-32 w-full relative overflow-hidden" style={{ backgroundColor: safeColor }}>
            ...
            <>
              <div className="mb-6">
                <h3 className="font-semibold mb-2 flex items-center">
                  <Sparkles className="h-4 w-4 mr-1" style={{ color: safeColor }} />
                  詳細な特徴
                </h3>
              ...
              <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {personalityType.compatibleTypes.map((type, index) => (
                    <Badge key={index} style={{ backgroundColor: safeColor, color: "white" }}>
                      {type}
                    </Badge>
                  ))}
                </div>
