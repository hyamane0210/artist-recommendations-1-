  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        {/* プレースホルダーアイテムの追加（アイテムが6個未満の場合） */}
        {visibleItems.length < 6 &&
          Array(6 - visibleItems.length)
            .fill(null)
            .map((_, index) => (
              <Card
                key={`placeholder-${index}`}
                className="overflow-hidden border-dashed hover:shadow-md transition-shadow"
              >
                <div className="relative aspect-square w-full bg-gray-50 flex items-center justify-center">
                  <Sparkles className="h-8 w-8 text-gray-400" />
                </div>
                <CardContent className="p-3">
                  <h3 className="font-medium text-sm mb-1">新しいおすすめを探す</h3>
                  <p className="text-xs text-muted-foreground line-clamp-2 h-10">
                    より多くのコンテンツを発見するには、「新しいおすすめを表示」ボタンをクリックしてください。
                  </p>
                </CardContent>
              </Card>
            ))}
