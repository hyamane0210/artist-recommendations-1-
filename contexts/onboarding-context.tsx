...
// 不要なインポートを削除
// import { hasVisitedInSession, markVisitedInSession, hasInitialRedirectDone, markInitialRedirectDone } from "@/utils/session-utils"

// 代わりにutils/session-utilsから直接インポート
...
export function OnboardingProvider({ children }: { children: ReactNode }) {
  ...
  // ローカルストレージとセッションストレージからオンボーディング状��を読み込む
  useEffect(() => {
    ...
    try {
      ...
      const visitedInSession = SessionUtils.hasVisitedInSession()
      ...
      if (pathname === "/welcome") {
        ...
        SessionUtils.markVisitedInSession()
        SessionUtils.markInitialRedirectDone()
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const initialRedirectDone = SessionUtils.hasInitialRedirectDone()
